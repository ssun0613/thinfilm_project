...
def setup(opt):
    ...
    model_caps = capsnet(opt.network_type).to(device)

    if opt.continue_train:
         model_caps = load_networks(model_caps, opt.checkpoint_name, device, net_name='model_caps', weight_path="/storage/mskim/checkpoint/")

    else:
        NotImplementedError('Invalid network_type')
    # -------------------------------------------- setup optimizer --------------------------------------------
    if opt.optimizer_name == 'Adam':
        optimizer = torch.optim.Adam(model_caps.parameters(), lr=opt.lr)
    elif opt.optimizer_name == 'RMSprop':
        optimizer = torch.optim.RMSprop(model_caps.parameters(), lr=opt.lr)
    else:
        optimizer = None
    ...
    return device, dataload_train, model_caps, optimizer, scheduler

def train_reg(config, writer, device, dataload_train, model_caps, optimizer, scheduler):
    ...
    for curr_epoch in range(config.opt.epochs):
        ...
        for batch_id, data in pbar:
            ...
            output = model_caps.forward(input)
        ...
        if (curr_epoch + 1 == 250) or (curr_epoch + 1 == 750) or (curr_epoch + 1 == 1000):
            torch.save({'model_caps': model_caps.state_dict(), 'optimizer': optimizer.state_dict()}, "/storage/mskim/checkpoint/{}/{}_{}.pth".format(config.opt.checkpoint_name,config.opt.checkpoint_name, curr_epoch+1))


def train_class(config, writer, device, dataload_train, model_caps, optimizer, scheduler):
    os.makedirs("/storage/mskim/checkpoint/{}".format(config.opt.checkpoint_name), exist_ok=True)
    ...
    for curr_epoch in range(config.opt.epochs):
        ...
        for batch_id, data in pbar:
            ...
            pred = model_caps.predicted(outs)
            acc = model_caps.accuracy(pred, one_hot)
        ...
        print(count_parameters(model_caps))
...
if __name__ == '__main__':
    ...
    device, dataload_train, model_caps, optimizer, scheduler = setup(config.opt)

    if config.opt.network_type == 'reg':
        train_reg(config, writer, device, dataload_train, model_caps, optimizer, scheduler)

    elif config.opt.network_type == 'class':
        train_class(config, writer, device, dataload_train, model_caps, optimizer, scheduler)
