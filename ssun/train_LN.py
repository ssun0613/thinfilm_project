...
from model.load_network import load_networks, count_parameters, calc_pr, precision_recall
...
def train_class(config, writer, device, dataload_train, model_LN, optimizer, scheduler):
    ...
    for curr_epoch in range(config.opt.epochs):
        ...
        for batch_id, data in pbar:
            ...
            pred, pred_pr = model_LN.predicted(outs)
            acc = model_LN.accuracy(pred, one_hot)

            TP, FP, FN = calc_pr(pred_pr, one_hot, TP, FP, FN)
        ...
        if (curr_epoch+1 == 250) or (curr_epoch+1 == 750) or (curr_epoch+1 == 1250) or (curr_epoch+1 == 1750):
            for idx in range(4):
                precision, recall = precision_recall(TP[idx], FP[idx], FN[idx])
                writer.add_scalar("precision_layer_{}".format(idx+1), precision, global_step)
                writer.add_scalar("recall_layer_{}".format(idx+1), recall, global_step)
