...
class Primarycaps(nn.Module):
    def __init__(self, in_features: int =904, out_features: int =4, cap_dims: int = 8):
        ...
        self.capsules = nn.ModuleList([nn.Linear(self.in_features, 256, bias=False) for _ in range(self.num_primary_caps)])


    def forward(self, x):
        out = [capsule(x) for capsule in self.capsules]
        out = torch.stack(out, dim=2) # torch.Size([200, 256, 226])
        num_routes = out.shape[1]*out.shape[2] # num_routes : 57856
        out = out.view(x.size(0), int(num_routes/8), 8)
...
class capsnet(nn.Module):
    def __init__(self, opt):
        ...
        self.digit_capsules = Digitcaps(in_dim=8, in_caps=7232, out_dim=30, out_caps=4, num_routing=3)
    ...
    def forward(self, input):
        ...
        self.output_primary = self.primary_layer(self.output_linear) # torch.Size([200, 50398, 8]) , ([200, 7232, 8])
